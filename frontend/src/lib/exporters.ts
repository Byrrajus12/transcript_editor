...
export function exportTXT(segments: Segment[], filename = "transcript.txt") {
  const blob = new Blob([serializeTXT(segments)], {
...
export function exportPDF(segments: Segment[], filename = "transcript.pdf") {
  const doc = new jsPDF({ unit: "pt", format: "letter" });
  const lineHeight = 16;
  const left = 48;
  let y = 64;
  const width = 515;
  const lines = serializeTXT(segments).split("\n");
